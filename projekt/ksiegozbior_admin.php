<?php session_start(); ?>
<!doctype html>
<html>
	<head>
		<meta charset="UTF-8">
		<style>
			h1 { color: #555B6E; text-align: center; font-size: 40px; }
			hr { border: 3px solid #555B6E; }
			body { background-color: #BEE3DB; }
			table { background-color: #89B0AE; width: 100%; }
			tr, td { text-align: center; color: #FAF9F9; border: 1px solid #FAF9F9; font-size: 20px;}
			a:link, a:visited, a:hover, a:active { text-decoration: none; color: #555B6E; font-size: 25px; }
		</style>
		<title>Biblioteka</title>
	</head>
	<body>
   <?php
$servername = "localhost";
$username = "id17168618_admin1";
$password = "c)XJDjo]}zLL4beK";
$dbname = "id17168618_biblioteka";

	// create connection
	$conn = new mysqli($servername, $username, $password, $dbname);

	//check connection
	if	($conn->connect_error) {
		die("Błąd połączenia: ".$conn->connect_error);
	}	
	$sql = "SELECT * FROM ksiazki";
	$result = $conn->query($sql);

	if ($result->num_rows > 0) {
      // output data of each row
      echo "<h1>BIBLIOTEKA - BAZA KSIĄŻEK - ".$_SESSION['imie']." ".$_SESSION['nazwisko']."</h1><hr>";
      echo "<a href='index.php'>Strona główna</a><br><a href='administrator.php'>Wstecz</a><br><br>";
      echo "<table><tr><td>ID książki</td><td>Tytuł</td><td>Gatunek</td><td>Autor</td><td>Dostępność</td></tr>";
      while ($row = $result->fetch_assoc()) {
			echo "<tr><td>".$row['ksiazka_id']."</td>
					<td>".$row['tytul']."</td>
					<td>".$row['gatunek']."</td>
					<td>".$row['autor']."</td>
					<td>".$row['dostepnosc']."</td>
					<td><a href='edytuj_ksiazke.php?id=".$row['ksiazka_id']."'>Edytuj</a></td>
					<td><a href='usun_ksiazke.php?id=".$row['ksiazka_id']."'>Usuń</a></td></tr>";
      }
      echo "</table>";
	} else {
		echo "0 results<br>";
		echo "<a href='index.php'>Strona główna</a>";
	}
	$conn->close();
   ?>
	</body>
</html>
